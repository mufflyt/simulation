...
test_that("mc_weighted_ci brackets the point estimate and is reproducible", {
  set.seed(99)
  access <- runif(200, 0, 50)
  est <- runif(200, 1e4, 5e4)
  se <- est * 0.05
  ci <- mc_weighted_ci(access, est, se, stat = "mean", B = 500, seed = 7)
  ...
  ci2 <- mc_weighted_ci(access, est, se, stat = "mean", B = 500, seed = 7)
  expect_equal(ci, ci2)
})

test_that("mc_weighted_ci restores the caller's RNG stream", {
  set.seed(123)
  before <- .Random.seed
  invisible(mc_weighted_ci(runif(10), runif(10, 1, 2), rep(0.1, 10), B = 50, seed = 5))
