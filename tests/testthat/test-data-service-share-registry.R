test_that("build_urogynecology_service_registry builds valid schema and rules", {
  ...
  expect_true(all(c("hcpcs", "service", "category", "evidence_quality", "effective_year", "setting_scope") %in% names(reg)))
  expect_gt(nrow(reg), 10)
  expect_true(all(reg$evidence_quality == "sourced"))
})
...
test_that("build_urogynecology_provider_taxonomy_registry builds valid taxonomy crosswalk", {
  ...
  expect_true(all(c("taxonomy_code", "provider_type", "specialty_group", "is_urps_specialist") %in% names(tax_reg)))
