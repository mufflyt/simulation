...
test_that("the summed rates are derived from the components, never typed", {
  ...
  expect_equal(
    unname(urpssim:::WU2011_SURGERY_RATE_PER_1000),
    urpssim:::WU2011_SURGERY_RATE_COMPONENTS$sui + urpssim:::WU2011_SURGERY_RATE_COMPONENTS$pop)
  expect_equal(names(urpssim:::WU2011_SURGERY_RATE_PER_1000),
               urpssim:::WU2011_SURGERY_RATE_COMPONENTS$age_band)
  expect_equal(names(urpssim:::WU2011_SURGERY_RATE_PER_1000), urpssim:::DEMAND_AGE_BANDS)
})
...
test_that("the published 60-79 rate is applied to both sub-bands", {
  ...
  cmp <- urpssim:::WU2011_SURGERY_RATE_COMPONENTS
...
test_that("surgery peaks at 60-79 and falls at 80+, unlike prevalence", {
  ...
  expect_gt(urpssim:::WU2011_SURGERY_RATE_PER_1000[["65-79"]],
            urpssim:::WU2011_SURGERY_RATE_PER_1000[["40-59"]])
})
...
test_that("an unrecognised age band is an error, not a silent drop", {
  ...
  good <- data.frame(year = 2025L, age_band = urpssim:::DEMAND_AGE_BANDS, female_pop = 1e6)
...
test_that("by_condition splits SUI and POP and conserves the total", {
  ...
  pop <- data.frame(year = c(2025L, 2030L), age_band = rep(urpssim:::DEMAND_AGE_BANDS, each = 2),
                    female_pop = 1e6)
