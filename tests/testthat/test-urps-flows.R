...
test_that("urps_p_active reproduces HWSM Exhibit 17 survival anchors for male", {
  ...
  expect_equal(urps_p_active(35, "male",  2), 0.985, tolerance = 0.015)
  expect_equal(urps_p_active(65, "male", 32), 0.550, tolerance = 0.050)
  expect_equal(urps_p_active(75, "male", 42), 0.120, tolerance = 0.030)
})

test_that("P(active) declines monotonically with age holding sex and entry fixed", {
  ages  <- seq(35, 80, by = 5)
  yrs   <- pmax(ages - 33, 0)
  probs <- urps_p_active(ages, "male", yrs)
...
test_that("female OR at reference age is approximately 0.77 (HWSM Exhibit 16)", {
  pm <- urps_p_active(65, "male",   32)
  pf <- urps_p_active(65, "female", 32)
...
test_that("female P(active) is lower than male at all retirement-zone ages", {
  ...
  yrs  <- ages - 33
  expect_true(all(urps_p_active(ages, "female", yrs) <
                  urps_p_active(ages, "male",   yrs)))
})

# ---- 3. Scenario shifts ------------------------------------------------------

test_that("retiring 2 years later raises P(active) at age 65", {
  p_base   <- urps_p_active(65, "male", 32, scenario_id = "baseline")
  p_later  <- urps_p_active(65, "male", 32, scenario_id = "retire_2yr_later")
  expect_gt(p_later, p_base)
})

test_that("retiring 2 years earlier lowers P(active) at age 65", {
  p_base    <- urps_p_active(65, "male", 32, scenario_id = "baseline")
  p_earlier <- urps_p_active(65, "male", 32, scenario_id = "retire_2yr_earlier")
...
test_that("NULL scenario_id produces the same result as baseline", {
  expect_equal(
    urps_p_active(65, "male", 32, scenario_id = NULL),
    urps_p_active(65, "male", 32, scenario_id = "baseline")
  )
})

test_that("unknown scenario_id warns and falls back to shift = 0", {
  expect_warning(
    p_unknown <- urps_p_active(65, "male", 32, scenario_id = "nonexistent_scenario"),
    regexp = "unknown scenario_id"
  )
  p_null <- urps_p_active(65, "male", 32, scenario_id = NULL)
...
test_that("age at or above terminal age (90) returns 0", {
  expect_equal(urps_p_active(90, "male",  57), 0)
  expect_equal(urps_p_active(95, "female", 62), 0)
})

test_that("age just below terminal age is non-zero", {
  expect_gt(urps_p_active(89, "male", 56), 0)
})
...
test_that("scalar and length-1 vector inputs produce identical output", {
  expect_equal(
    urps_p_active(65, "female", 32),
    urps_p_active(c(65), c("female"), c(32))
  )
})
...
test_that("vector recycling works for sex when age is a vector", {
  ...
  p_vec <- urps_p_active(ages, "female", yrs)
  p_ind <- vapply(seq_along(ages),
                  function(i) urps_p_active(ages[i], "female", yrs[i]),
                  numeric(1))
  expect_equal(p_vec, p_ind, tolerance = 1e-12)
})

test_that("output length matches the longest input", {
  expect_length(urps_p_active(c(40, 50, 60), "male", 10),        3)
  expect_length(urps_p_active(50, c("male", "female"), c(10, 15)), 2)
})
...
test_that("custom coef list changes the output predictably", {
  ...
  p <- urps_p_active(c(40, 65, 75), "male", c(7, 32, 42), coef = flat_coef)
