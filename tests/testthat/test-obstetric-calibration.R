...
test_that("older cohorts carry more vaginal births than younger (falling parity + rising cesarean)", {
  ...
  vo <- mean(simulate_lifecourse_demand(pa_old, 2040, seed = 1, n = 8000,
                             pathway = valid_pathway())$person_years$vaginal_births)
  vy <- mean(simulate_lifecourse_demand(pa_yng, 2040, seed = 1, n = 8000,
                             pathway = valid_pathway())$person_years$vaginal_births)
  expect_gt(vo, vy)
})
