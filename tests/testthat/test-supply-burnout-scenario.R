# Guards for the burnout_reduction supply scenario (early-career-attrition model).
#
# Burnout reduction acts on the AGE-FLAT early-exit (career-change) hazard, not
# the age-graded retirement curve, so it is a `career_change_multiplier` scenario
# threaded into run_supply_microsimulation()'s career_change_hazard -- NOT a
# retire-later `retirement_shift_years` lever. These tests also guard against a
# regression back to the retention (retire-later) representation.

test_that("burnout_reduction is an early-career-attrition scenario, not retire-later", {
  reg <- local_supply_scenario_registry(baseline_entrants = 55)
  expect_true("burnout_reduction" %in% names(reg))
  b <- reg$burnout_reduction
  expect_equal(b$career_change_multiplier, 0.75)   # 25% fewer early-career exits
  expect_lt(b$career_change_multiplier, 1)
  # It must act on the early-exit hazard, NOT shift the retirement curve.
  expect_equal(b$retirement_shift_years, 0)
  expect_equal(b$hours_multiplier, 1.00)
  expect_null(b$hazard_mult)                        # no forbidden curve multiplier
...
test_that("run_supply_microsimulation exposes career_change_hazard and responds to it", {
  ...
  # All agents start at 38 and never reach the age-50 retirement regime over the
  # horizon, so the ONLY exit process is the career-change (burnout) hazard -- the
  # early-career attrition a retirement-curve shift could not touch.
