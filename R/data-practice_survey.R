...
geographic_access_status <- function() {
  ...
  components <- tibble::tribble(
    ...
    "wait_time_anchor",      "PRESENT",  sprintf("Observed URPS wait %.1f %s from %s. Supplied by urps_observed_wait_days(); populates the wait_time target that ships unpopulated. The access layer is business-day native: wait_scale, wait_time and the %g-%s appointment window all carry this unit.", .wa$business_days, .wa$unit, sub("\\..*", "", .wa$citation), urps_appointment_window(), .wa$unit),
