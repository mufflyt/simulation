...
unresolved_calibration_items <- function() {
  tibble::tribble(
    ...
    "geographic_access",  FALSE,     FALSE,                 NA,           "data integration",  "Not in any reported result: R/geography-spatial_access_e2sfca is loaded and called by nothing. Cannot be wrong, only absent. Basis for every distributional claim the model might make. See geographic_access_status().",
...
geographic_access_status <- function() {
  components <- tibble::tribble(
    ...
    "demand_machinery",      "WIRED",    "R/geography-demand: tract_need_from_population(), demand_by_travel_band(), need_weighted_access(), isochrone_demand_from_tracts(). Called from scripts/run_demand_pipeline.R.",
    "supply_machinery",      "DORMANT",  "R/geography-spatial_access_e2sfca: compute_access(), match_points_to_isochrones(), compare_access_methods(), access_moe_ci(). Present in the orchestrator's module load list and called by NOTHING outside tests.",
  ...
  list(
    ...
    ordering_trap = paste(
      "Do NOT wire R/geography-spatial_access_e2sfca first. Running it before provider coordinates exist",
    ...
    resolved_by = c("provider point locations geocoded from roster NPIs",
                    ...
                    "R/geography-spatial_access_e2sfca called from the orchestrator, not merely loaded",
