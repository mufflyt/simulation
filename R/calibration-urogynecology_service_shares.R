...
.service_share_require_counts <- function(events) {
  ...
  if (base::any(!base::is.finite(counts)) || base::any(counts < 0)) {
    base::stop(
      "service_events must be finite and nonnegative.",
      call. = FALSE
    )
  }
  base::invisible(TRUE)
...
#'
#' Selects the amount of service-level shrinkage independently for each
#' service by minimizing multinomial cross-entropy in the latest held-out year.
#' This is the calibrated replacement for the historical fixed prior strength.
#'
#' @param events Provider-group counts by service, condition, and year.
...
select_service_share_concentration <- function(
  ...
  if (!base::is.numeric(alpha_grid) || base::length(alpha_grid) < 1L ||
    ...
    base::stop(
      "alpha_grid must contain positive finite values.",
      call. = FALSE
    )
  }
  ...
  scores <- purrr::map_dfr(
    ...
    function(service_name) {
      ...
      purrr::map_dfr(alpha_grid, function(alpha_value) {
        ...
        total_log_score <- base::sum(condition_score$log_score)
        tibble::tibble(
          service = service_name,
          holdout_year = holdout_year,
          alpha = alpha_value,
          log_score = total_log_score,
          holdout_events = total_events,
          cross_entropy = -total_log_score / total_events
        )
      })
    }
  )
  ...
  base::message(
    ...
    scales::comma(base::nrow(selected)), " services."
  )
...
.service_share_rdirichlet <- function(alpha, draws) {
  if (base::any(!base::is.finite(alpha)) || base::any(alpha <= 0)) {
    base::stop(
      "Dirichlet alpha values must be positive and finite.",
      call. = FALSE
    )
  }
...
#' @param selected_alpha Selected service-specific concentration table.
...
calibrate_service_share_model <- function(
  ...
  bundle <- base::list(
    ...
    provenance = base::list(
      ...
      cms_evidence_sha256 = if (base::is.null(cms_evidence)) {
        NA_character_
      } else {
        digest::digest(cms_evidence, algo = "sha256", serialize = TRUE)
      },
      chia_evidence_sha256 = if (base::is.null(chia_evidence)) {
        NA_character_
      } else {
  ...
  base::message(
    ...
    scales::comma(dplyr::n_distinct(bundle$share_draws$service)),
    " services, ",
    scales::comma(dplyr::n_distinct(bundle$share_draws$draw_id)),
    " joint draws."
  )
  bundle
}
...
validate_service_share_bundle <- function(bundle, tolerance = 1e-8) {
  ...
  if (base::any(!base::is.finite(draws$share)) ||
    ...
    base::stop(
      "Every service-share draw must have share in [0, 1].",
      call. = FALSE
    )
  }
  ...
  if (base::any(base::abs(sums$share_sum - 1) > tolerance)) {
    base::stop(
      "Calibrated provider shares do not sum to one.",
      call. = FALSE
    )
  }
  ...
  if (base::any(!base::is.finite(alpha)) || base::any(alpha <= 0)) {
    base::stop(
      "Selected alpha values must be positive and finite.",
      call. = FALSE
    )
  }
  base::invisible(TRUE)
