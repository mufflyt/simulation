...
#' found were held fixed:
#' \enumerate{
#'   \item an empirical entrant process allowing a recent regime shift in level
#'     and slope, and
#'   \item an age-specific departure hazard fitted from observed ABOG events.
#' }
#' Both processes are propagated through Monte Carlo parameter draws rather than
#' held at a point estimate.
#'
...
#' @export
calibrate_urps_supply_dynamics <- function(
    entrant_tbl,
    departure_tbl,
    forecast_years,
    n_draws = 5000L,
    ...
    seed = 42L) {
  base::message(
    "Starting URPS supply-dynamics calibration."
  )
  base::message(
    "Entrant rows: ",
    base::format(
      base::nrow(entrant_tbl),
      big.mark = ","
    )
  )
  base::message(
    "Departure provider-years: ",
    base::format(
      base::nrow(departure_tbl),
      big.mark = ","
    )
  )
  base::message(
  ...
  required_entrant_cols <- c(
    "year",
    "entrants"
  )
  required_departure_cols <- c(
    "provider_id",
    "year",
    "age",
    "departed"
  ...
  if (!base::all(
    required_entrant_cols %in%
      base::names(entrant_tbl)
  )) {
    base::stop(
      "entrant_tbl must contain year and entrants."
    )
  }
  if (!base::all(
    required_departure_cols %in%
      base::names(departure_tbl)
  )) {
    base::stop(
      paste0(
        "departure_tbl must contain provider_id, ",
        "year, age, and departed."
      )
    )
  }
  base::set.seed(seed)
  base::message(
    "Preparing empirical entrant series."
  )
  ...
  if (base::nrow(entrant_clean) < 6L) {
    base::stop(
      "At least six entrant years are recommended."
    )
  }
  entrant_min_year <- base::min(
    entrant_clean$year
  )
  entrant_max_year <- base::max(
    entrant_clean$year
  )
  base::message(
    "Entrant observation window: ",
    entrant_min_year,
    "-",
    entrant_max_year,
    "."
  ...
  base::message(
    "Testing ",
    base::length(candidate_breaks),
    " candidate entrant regime breaks."
  )
  entrant_break_models <- purrr::map(
    candidate_breaks,
    function(break_year) {
      analysis_tbl <- entrant_clean |>
        dplyr::mutate(
          time = .data$year -
            entrant_min_year,
          post_break = base::as.integer(
            .data$year >= break_year
          ),
          time_after_break = base::pmax(
            0,
            .data$year - break_year
          )
        )
  ...
  if (base::all(
    !base::is.finite(
      entrant_break_models$aic
    )
  )) {
    base::message(
      paste0(
        "Quasi-Poisson AIC unavailable; ",
        "selecting break by squared prediction error."
      )
    )
    entrant_break_models <- entrant_break_models |>
      dplyr::mutate(
        prediction_error = purrr::map_dbl(
          .data$model,
          function(model_fit) {
    ...
    selected_break_row <- entrant_break_models |>
      dplyr::slice_min(
        order_by = .data$prediction_error,
        n = 1L,
        with_ties = FALSE
      )
  } else {
  ...
  selected_break <- selected_break_row |>
    dplyr::pull(.data$break_year)
  selected_entrant_model <- selected_break_row |>
    dplyr::pull(.data$model) |>
    purrr::pluck(1L)
  base::message(
    "Selected entrant regime break: ",
    selected_break,
    "."
  )
  entrant_coef <- stats::coef(
    selected_entrant_model
  )
  entrant_vcov <- stats::vcov(
    selected_entrant_model
  )
  entrant_dispersion <- stats::summary.glm(
    selected_entrant_model
  )$dispersion
  base::message(
    "Entrant overdispersion estimate: ",
    base::format(
      entrant_dispersion,
      digits = 3
    ),
  ...
  entrant_beta_draws <- MASS::mvrnorm(
    n = n_draws,
    mu = entrant_coef,
    Sigma = entrant_vcov
  )
  entrant_beta_draws <- tibble::as_tibble(
    entrant_beta_draws,
    .name_repair = "unique"
  )
  base::names(
  ...
  entrant_beta_draws <- entrant_beta_draws |>
    dplyr::mutate(
      draw = dplyr::row_number()
    )
  base::message(
    "Estimating ABOG departure hazard."
  )
  departure_clean <- departure_tbl |>
    ...
    dplyr::filter(
      ...
      .data$departed %in% c(0L, 1L)
    )
  base::message(
    "Eligible departure provider-years: ",
    base::format(
      base::nrow(departure_clean),
      big.mark = ","
    ),
    "."
  )
  ...
  if (base::sum(
    ...
    base::warning(
      paste0(
        "Fewer than 20 observed departures. ",
        "Departure-hazard uncertainty may be large."
      )
    )
  }
  departure_model <- stats::glm(
    ...
    data = departure_clean
  )
  departure_coef <- stats::coef(
    departure_model
  )
  departure_vcov <- stats::vcov(
    departure_model
  )
  departure_beta_draws <- MASS::mvrnorm(
    n = n_draws,
    mu = departure_coef,
    Sigma = departure_vcov
  )
  departure_beta_draws <- tibble::as_tibble(
    departure_beta_draws,
    .name_repair = "unique"
  )
  base::names(
    departure_beta_draws
  ...
  departure_beta_draws <- departure_beta_draws |>
    dplyr::mutate(
      draw = dplyr::row_number()
    )
  base::message(
    "Generating entrant forecast draws."
  )
  entrant_future_tbl <- tibble::tibble(
  ...
  entrant_design <- stats::model.matrix(
    ~ time + post_break + time_after_break,
    data = entrant_future_tbl
  )
  entrant_linear_predictor <- entrant_design %*%
    base::t(
      base::as.matrix(
        entrant_beta_draws |>
          dplyr::select(
  ...
  entrant_mean_matrix <- base::exp(
    entrant_linear_predictor
  )
  ...
  entrant_summary <- entrant_draws |>
    dplyr::group_by(.data$year) |>
    dplyr::summarise(
      mean_entrants = base::mean(
        .data$entrants
      ),
      sd_entrants = stats::sd(
        .data$entrants
      ),
      median_entrants = stats::median(
        .data$entrants
      ),
      p25_entrants = stats::quantile(
        .data$entrants,
        probs = 0.25,
        names = FALSE
      ),
      p75_entrants = stats::quantile(
        .data$entrants,
        probs = 0.75,
        names = FALSE
      ),
      p025_entrants = stats::quantile(
        .data$entrants,
        probs = 0.025,
        names = FALSE
      ),
      p975_entrants = stats::quantile(
        .data$entrants,
        probs = 0.975,
        names = FALSE
      ),
  ...
  base::message(
    "Generating age-specific departure-hazard draws."
  )
  age_grid <- base::seq(
    from = min_retirement_age,
    to = 90,
    by = 1
  )
  reference_year <- base::max(
    departure_clean$year
  )
  departure_prediction_tbl <- tibble::tibble(
    age = age_grid,
    year = reference_year
  )
  # Hold the year effect at the most recent observed level and evaluate the
  # hazard across the age grid. The prediction frame carries a single year, so
  # `factor(year)` would collapse to one level and model.matrix() would throw
  # "contrasts can be applied only to factors with 2 or more levels"; passing
  # the fitted model's xlevels restores the full factor and the reference-year
  # dummies line up with the coefficient names.
  departure_design <- stats::model.matrix(
    ...
    data = departure_prediction_tbl,
    xlev = departure_model$xlevels
  )
  common_terms <- base::intersect(
    base::colnames(
      departure_design
    ),
    base::colnames(
  ...
  departure_design_common <- departure_design[
    ,
    common_terms,
    drop = FALSE
  ]
  departure_coef_matrix <- base::as.matrix(
    departure_beta_draws |>
      dplyr::select(
        dplyr::all_of(
          common_terms
  ...
  departure_linear_predictor <-
    departure_design_common %*%
    base::t(
      departure_coef_matrix
    )
  departure_probability_matrix <-
    1 -
    base::exp(
      -base::exp(
        departure_linear_predictor
  ...
  departure_hazard_draws <-
    ...
    tidyr::pivot_longer(
      ...
      names_to = "draw_name",
      values_to = "departure_probability"
    ) |>
  ...
  departure_summary <- departure_hazard_draws |>
    dplyr::group_by(.data$age) |>
    dplyr::summarise(
      mean_probability = base::mean(
        .data$departure_probability
      ),
      sd_probability = stats::sd(
        .data$departure_probability
      ),
      median_probability = stats::median(
        .data$departure_probability
      ),
      p25_probability = stats::quantile(
        .data$departure_probability,
        probs = 0.25,
        names = FALSE
      ),
      p75_probability = stats::quantile(
        .data$departure_probability,
        probs = 0.75,
        names = FALSE
      ),
      p025_probability = stats::quantile(
        .data$departure_probability,
        probs = 0.025,
        names = FALSE
      ),
      p975_probability = stats::quantile(
        .data$departure_probability,
        probs = 0.975,
        names = FALSE
      ),
  ...
  base::message(
    "Estimated empirical retirement hazard CV: ",
    base::format(
      overall_hazard_cv,
      digits = 3
    ),
  ...
  calibration_diagnostics <- tibble::tibble(
    metric = c(
      "entrant_break_year",
      "entrant_last_observed_year",
      "departure_reference_year",
      "observed_departures",
      "retirement_hazard_cv"
    ),
    value = c(
      selected_break,
      entrant_max_year,
  ...
  base::message(
    "Supply-dynamics calibration complete."
  )
  base::list(
    entrant_model = selected_entrant_model,
    entrant_break_year = selected_break,
    entrant_draws = entrant_draws,
    entrant_summary = entrant_summary,
    departure_model = departure_model,
    departure_hazard_draws =
      departure_hazard_draws,
...
#'
#' @description
#' Evolves a provider population forward one year using an empirical
#' ABOG-derived departure-hazard draw and an empirical entrant draw. Ages every
#' provider by one year, applies the draw's age-specific departure probability,
#' removes departed providers, and appends the draw's entrant count.
#'
...
#' @concept supply
#' @export
advance_urps_supply_one_year <- function(
    provider_tbl,
    simulation_year,
    draw,
    calibration,
  ...
  # Inside dplyr::filter() a bare `draw` is resolved against the DATA first, so
  # `.data$draw == draw` would compare the column to itself (all TRUE, no
  # filtering). Capture the argument under a name that is not a column so the
  # comparison reads the argument.
  target_draw <- draw
  ...
  # own `draw` column into the provider roster would collide on the next year's
  # join and fan out.
  hazard_tbl <- calibration$departure_hazard_draws |>
    dplyr::filter(
      .data$draw == target_draw
    ) |>
    dplyr::select(
      "age",
      "departure_probability"
  ...
  if (base::length(
    ...
    base::stop(
      paste0(
        "Expected exactly one entrant count ",
        "for the requested year and draw."
      )
    )
  }
  base::message(
    "New independent URPS entrants: ",
    base::format(
      entrant_count,
      big.mark = ","
    ),
  ...
  if (entrant_count > 0L) {
    ...
    updated_providers <- dplyr::bind_rows(
      surviving_providers,
      new_providers
    )
  } else {
...
#' @concept supply
#' @export
backtest_urps_supply_calibration <- function(
    base_provider_tbl,
    observed_supply_tbl,
    calibration,
    start_year = 2020L,
    ...
    seed = 42L) {
  base::message(
    "Starting URPS supply back-test: ",
    start_year,
    " -> ",
    end_year,
    "."
  ...
  base::set.seed(seed)
  backtest_draws <- purrr::map_dfr(
    base::seq_len(
      n_draws
    ),
    function(draw_id) {
      provider_state <- base_provider_tbl
      yearly_counts <- tibble::tibble(
        draw = draw_id,
        year = start_year,
        predicted_supply =
          base::nrow(
  ...
  base::message(
    "Summarizing back-test distributions."
  )
  forecast_summary <- backtest_draws |>
    dplyr::group_by(.data$year) |>
    dplyr::summarise(
      mean_supply = base::mean(
        .data$predicted_supply
      ),
      sd_supply = stats::sd(
        .data$predicted_supply
      ),
      median_supply = stats::median(
        .data$predicted_supply
      ),
      p25_supply = stats::quantile(
        .data$predicted_supply,
        probs = 0.25,
        names = FALSE
      ),
      p75_supply = stats::quantile(
        .data$predicted_supply,
        probs = 0.75,
        names = FALSE
      ),
  ...
  calibration_metrics <- forecast_summary |>
    ...
    dplyr::summarise(
      ...
      median_absolute_percent_error =
        stats::median(
          .data$absolute_percent_error,
          na.rm = TRUE
        ),
      p25_absolute_percent_error =
        stats::quantile(
          .data$absolute_percent_error,
          probs = 0.25,
          na.rm = TRUE,
  ...
  start_observed <- forecast_summary |>
    dplyr::filter(
      .data$year == start_year
    ) |>
    dplyr::pull(
  ...
  start_predicted <- forecast_summary |>
    dplyr::filter(
      .data$year == start_year
    ) |>
    dplyr::pull(
  ...
  end_predicted <- forecast_summary |>
    dplyr::filter(
      .data$year == end_year
    ) |>
    dplyr::pull(
  ...
  observed_change <- end_observed -
    start_observed
  predicted_change <- end_predicted -
    start_predicted
  slope_error <- predicted_change -
    observed_change
  direction_text <- dplyr::case_when(
    slope_error < 0 ~
      "under-predicted supply growth",
    slope_error > 0 ~
      "over-predicted supply growth",
    TRUE ~
      "matched observed supply growth"
  )
  summary_sentence <- base::sprintf(
    paste0(
      "From %d to %d, observed URPS supply changed by ",
      "%s clinicians, while the calibrated model predicted ",
      "a mean change of %s; the model %s by %s clinicians. ",
    ...
    100 *
      calibration_metrics$coverage_95
  )
  base::message(
    summary_sentence
  )
...
#' @concept supply
#' @export
decompose_urps_forecast_miss <- function(
    entrant_tbl,
    departure_tbl,
    base_provider_tbl,
    observed_supply_tbl,
  ...
  fit_regime <- function(regime_entrant, regime_departure, label) {
    ...
    cal <- calibrate_urps_supply_dynamics(
      entrant_tbl = regime_entrant,
      departure_tbl = regime_departure,
    ...
    bt <- backtest_urps_supply_calibration(
      base_provider_tbl = base_provider_tbl,
      observed_supply_tbl = observed_supply_tbl,
      calibration = cal,
  ...
  train_entrant <- entrant_tbl |>
    dplyr::filter(
      base::as.integer(.data$year) <= start_year
    )
  train_departure <- departure_tbl |>
    dplyr::filter(
      base::as.integer(.data$year) <= start_year
    )
  ...
  slope_of <- function(bt) {
    ...
    obs_change <- fs$observed_supply[fs$year == end_year] -
      fs$observed_supply[fs$year == start_year]
    pred_change <- fs$mean_supply[fs$year == end_year] -
      fs$mean_supply[fs$year == start_year]
    pred_change - obs_change
  }
  ...
  metric_row <- function(regime_label, bt, slope_error) {
    ...
    tibble::tibble(
      regime = regime_label,
      signed_percent_bias = m$mean_percent_error,
  ...
  decomposition <- tibble::tibble(
    component = c(
      "unforeseeable_regime_break",
      "deficient_entrant_model"
    ),
    slope_error_clinicians = c(
      train_slope_error - oracle_slope_error,
      oracle_slope_error
    )
  )
  ...
  summary_sentence <- base::sprintf(
    paste0(
      "Over %d-%d the leakage-free model missed the supply slope by %.1f ",
      "clinicians; hindsight (oracle) narrows that to %.1f. Of the miss, ",
      "%.1f is attributable to an unforeseeable regime break and %.1f to a ",
      "deficient entrant model. Train 95%% coverage %.1f%%, oracle %.1f%%."
    ),
    start_year,
    end_year,
    train_slope_error,
    ...
    train_slope_error - oracle_slope_error,
    oracle_slope_error,
    100 * train$backtest$metrics$coverage_95,
    100 * oracle$backtest$metrics$coverage_95
  )
