...
# [disambiguated] was a second, DIVERGENT definition of simulate_supply(); renamed to
# simulate_supply_variant1() so it no longer silently shadows the definition below. Verify which is wanted.
simulate_supply_variant1 <- function(years, new_per_year, retire_per_year, initial_supply, scenario_label) {
...
census_path <- data_raw_path("np2023-t2.xlsx")
...
#
# brfss <- haven::read_xpt(external_path("tyler/LLCP2021.XPT"))
...
# load(external_path("ICPSR_04368/DS0001/04368-0001-Data.rda"))
#
...
# Original Function -----
# [deduplicated] process_involea(): identical earlier definition removed; the surviving definition is below.
...
readr::write_csv(swan_ui_modeling, swan_path("swan_ui_modeling.csv"))
...
env1 <- new.env()
load(swan_path("28762-0001-Data.rda"), envir = env1)
...
env2 <- new.env()
load(swan_path("04368-0001-Data.rda"), envir = env2)
...
write_csv(swan_longitudinal_data, swan_path("swan_longitudinal_data_for_dpmm.csv"))
...
da04368.0001 <- load(swan_path("04368-0001-Data.rda"))
