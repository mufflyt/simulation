...
run_complete_calibration_example <- function() {
  ...
  swan_file_path <- swan_path("merged.rds")
...
run_fixed_calibration_workflow <- function() {
  ...
  if (!file.exists(swan_file_path)) {
    ...
    possible_paths <- c(
      ...
      "./data/SWAN/merged.rds",
      swan_path("merged_longitudinal_data.rds"),
      swan_path("merged_longitudinal_data.rds")
    )
...
if (!requireNamespace("stringr", quietly = TRUE)) {
  ...
  stop("Required package 'stringr' is not installed. Install it and re-run.", call. = FALSE)
}
