...
# NOTE: this file previously read merged_longitudinal_data.rds AT SOURCE TIME,
# so `source()`ing it failed on any machine without that exact file. Load the
# data explicitly before calling the functions below:
#   merged_longitudinal_data <- readr::read_rds(swan_path("merged_longitudinal_data.rds", must_work = TRUE))
...
#' #'   longitudinal data. Default is swan_path("merged_longitudinal_data.rds").
...
#'     swan_file_path = swan_path("merged_longitudinal_data.rds"),
#'     baseline_visit_number = 0,
#'     validation_visit_numbers = c(1, 2, 3, 4, 5),
#'     max_followup_years = 10,
#'     n_simulations = 500,
...
raw_swan_data <- readRDS(swan_path("merged_longitudinal_data.rds"))
